...
import autograd.scipy.misc as smi # special
...
class data_obj(object):
    ...
    def __init__(self,angle,F,S,dfof,nbefore,nafter):
        self.stim_id = angle.copy() # (y,N) shape, where y is the number of parameters varied
        if len(self.stim_id.shape)<2:
            self.stim_id = self.stim_id[np.newaxis]
        self.F = F.copy()
...
#        self.g = np.array(g)
    ...
    def p_s_given_r(self,s,r,floor=1e-8):
        # bin-wise likelihood of r given s spikes in each bin
        # n x T x N
        if self.oversampled:
            rhat = snd.convolve(r,self.sampwt)[self.sampmat].reshape(s.shape)
            return self.floor_it(np.exp(-rhat)*rhat**s/np.math.factorial(s),floor)
        else:
            return self.floor_it(np.exp(-r)*r**s/np.math.factorial(s),floor)

    def log_p_s_given_r(self,s,r,floor=1e-8):
        # bin-wise likelihood of r given s spikes in each bin
        # n x T x N
        if self.oversampled:
            rhat = snd.convolve(r,self.sampwt)[self.sampmat].reshape(s.shape)
            #return -rhat + s*np.log(rhat) - np.log(np.math.factorial(s))
            return -rhat + s*np.log(rhat) - np.log(ssp.factorial(s))
        else:
            #return -r + s*np.log(r) - np.log(np.math.factorial(s))
            return -r + s*np.log(r) - np.log(ssp.factorial(s))
    
    def p_F_given_s(self,s,floor=1e-8):
        ...
        p_raw = np.exp(-0.5*((self.F-self.b)[:,self.p:]-self.a*s-prev.sum(0))**2/self.noise)
        return self.floor_it(p_raw,floor)
    ...
    def L_of_Theta(self,theta,fn_obj):
        ...
        log_ell = np.array([self.log_pFs[s] + self.log_p_s_given_r(s,r) for s in range(self.smax)])
        return np.sum(smi.logsumexp(log_ell,axis=0))
        ### return self

    def dLdTheta(self,theta,fn_obj):
        return egrad(lambda x: self.L_of_Theta(x,fn_obj))(theta)
        ### return self.dL(theta)
    
    ##def dLdTheta(self,theta,fn_obj,fudge=0):
