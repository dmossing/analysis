...
gd.LoadImages.type = uicontrol(...
    ...
    'Position',             [.5,.9,.49,.1]);
% frames to load
gd.LoadImages.framestext = uicontrol(...
    'Style',                'text',...
    'String',               'Frames:',...
    'Parent',               gd.LoadImages.panel,...
    'Units',                'normalized',...
...
gd.LoadImages.frames = uicontrol(...
    ...
    'Position',             [.25,.8,.25,.1]);
% channels to load
gd.LoadImages.channelstext = uicontrol(...
    'Style',                'text',...
    'String',               'Channels:',...
    'Parent',               gd.LoadImages.panel,...
    'Units',                'normalized',...
...
% histogram equalizer
gd.DataSelection.histeq = uicontrol(...
    'Style',                'radio',...
    ...
    switch gd.state.UIstate
        ...
        case 'ready' %base state
            ...
            switch eventData.Key
                ...
                case 'm'
                    if strcmp(gd.LoadExperiment.MC.Enable, 'on')
...
for c = 1:numel(gd.state.channels)
    ...
    if gd.DataSelection.histeq.Value
        temp = adapthisteq(temp, 'NumTiles', [16 16], 'Distribution', 'Exponential');
    end
...
if iscell(eventdata) % arguments specified in function call
    ...
    if isempty(Frames) || (ischar(Frames) && any(strcmp(Frames, 'all')));
        Frames = [1 inf];
    else
    ...
    if isempty(Channels) || (ischar(Channels) && any(strcmp(Channels, 'all')));
        Channels = [1 inf];
    else
...
%     gd.state.(gd.Images.Computations.Names{1}).clim(c).limits=[min(img(:)),max(img(:))];
    gd.state.(gd.Images.Computations.Names{1}).clim(c).limits = [0, double(intmax(gd.Images.info.Precision))];
...
[f,p] = uiputfile({'*.png','PNG file (*.png)';'*.tif','tif file (*.tif)';...
...
% Save ROI data
save('\\128.32.173.33\imaging\STIM\HoloRequest\ROIdata.mat',VariablesToSave{:},'-mat')
set(gcf, 'pointer', 'arrow')
