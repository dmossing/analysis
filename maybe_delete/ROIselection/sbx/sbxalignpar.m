...
% T - optimal translation for each frame
    ...
    A = sbxchan0(sbxreadpacked(fname,1,1)); % use only green channel info
...
function [m,v,T] = sbxalignsub(fname,idx,rg1,rg2,thestd,gl,l)
    ...
    if(length(idx)==1)
        ...
        A = double(sbxchan0(sbxreadpacked(fname,idx(1),1)));
    ...
    elseif (length(idx)==2)
        ...
        B = double(sbxchan0(sbxreadpacked(fname,idx(2),1)));
