...
%     sbxpacksignals(fname); %Takes about 10 minutes
    ...
    z = sbxchan0(sbxreadpacked(fname,0,1)); % align based only on green channel
    ...
    parfor jj = 1:info.max_idx
        ...
        z = double(sbxchan0(sbxreadpacked(fname,jj-1,1))); % align based only on green channel
...
%     thestd = zeros(size(s));
%     for i=1:size(s,3)
%         thestd(:,:,i) = medfilt2(s(:,:,i),[31,31],'symmetric');
%     end
    ...
    parfor jj = 1:info.max_idx
        ...
        z = single(sbxchan0(sbxreadpacked(fname,jj-1,1)));
    ...
    parfor jj = 1:info.max_idx
        ...
        z = double(sbxchan0(sbxreadpacked(fname,jj-1,1)));
