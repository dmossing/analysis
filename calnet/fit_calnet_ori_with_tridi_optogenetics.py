...
reload(sim_utils)
import calnet.utils
import calnet.fitting_spatial_feature_opto_bidi
import opto_utils
...
def initialize_W(Xhat,Yhat,scale_by=0.2):
    ...
    nQ = Yhat[0][0].shape[1]
    nN = Yhat[0][0].shape[0]
    YYhat = calnet.fitting_spatial_feature_opto_bidi.flatten_nested_list_of_2d_arrays(Yhat)
    XXhat = calnet.fitting_spatial_feature_opto_bidi.flatten_nested_list_of_2d_arrays(Xhat)
    Wmy0 = np.zeros((nQ,nQ))
    Wmx0 = np.zeros((nP,nQ))
...
def fit_weights_and_save(weights_file,ca_data_file='rs_vm_denoise_200605.npy',vip_silencing_data_file='vip_halo_data_for_sim.npy',vip_activation_data_file='vip_chrimson_data_for_sim.npy',sst_silencing_data_file='sst_halo_data_for_sim.npy',constrain_wts=None,allow_var=True,fit_s02=True,constrain_isn=True,l2_penalty=0.01,init_noise=0.1,init_W_from_lsq=False,scale_init_by=1,init_W_from_file=False,init_file=None):
    ...
    shapes = [(nP,nQ),(nQ,nQ),(nP,nQ),(nQ,nQ),(nQ,),(nQ,),(1,),(nQ,),(nN,nT*nS*nP),(nN,nT*nS*nP),(nN,nT*nS*nQ),(nN,nT*nS*nQ),(1,),(1,)]
    ...
    Yhat_opto = opto_dict['Yhat_opto']
    Yhat_opto = Yhat_opto/np.nanmax(Yhat_opto,0)[np.newaxis,:]
    print(Yhat_opto.shape)
    h_opto = opto_dict['h_opto']
    ...
    from importlib import reload
    reload(calnet)
    #reload(calnet.fitting_spatial_feature_opto_tridi)
    reload(sim_utils)
    ...
    YYhat = calnet.fitting_spatial_feature_opto_tridi.flatten_nested_list_of_2d_arrays(Yhat)
    XXhat = calnet.fitting_spatial_feature_opto_tridi.flatten_nested_list_of_2d_arrays(Xhat)
