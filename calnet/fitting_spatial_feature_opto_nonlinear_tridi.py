...
def fit_W_sim(Xhat,Xpc_list,Yhat,Ypc_list,dYY,pop_rate_fn=None,pop_deriv_fn=None,neuron_rate_fn=None,W0list=None,bounds=None,dt=1e-1,perturbation_size=5e-2,niter=1,wt_dict=None,eta=0.1,compute_hessian=False,l2_penalty=1.0,constrain_isn=False,opto_mask=None,nsize=6,ncontrast=6):
    ...
    def parse_W(W):
        # Wmx,Wmy,Wsx,Wsy,s02,K,kappa,T,XX,XXp,YY,Eta,Xi,h1,h2,h3,Eta1,Eta2,Eta3
        shapes = [(nP,nQ),(nQ,nQ),(nP,nQ),(nQ,nQ),(nQ,),(nQ,),(1,),(nQ,),(nN,nT*nS*nP),(nN,nT*nS*nP),(nN,nT*nS*nQ),(nN,nT*nS*nQ),(1,),(1,),(1,),(nN,nT*nS*nQ),(nN,nT*nS*nQ),(nN,nT*nS*nQ)]
...
#             WWx,WWy = [gen_Weight(W,k,kappa) for W in [Wx,Wy]]
        ...
        def compute_f_fprime_t_(W,perturbation):
            Wmx,Wmy,Wsx,Wsy,s02,k,kappa,T,XX,XXp,Eta,Xi,h1,h2,h3,Eta1,Eta2,Eta3 = parse_W(W)
            ...
            def dYYdt(YY,thisEta,thisXi):
                return -YY + compute_f_(thisEta,thisXi,s02)
            for t in range(niter):
                Etaprime = resEta + u_fn(XX,YY,Wmx,Wmy,k,kappa)
                Xiprime = resXi + u_fn(XX,YY,Wmx,Wmy,k,kappa)
                YY = YY + dt*dYYdt(YY,Etaprime,Xiprime)
                
            YYprime = compute_fprime_(Etaprime,Xiprime,s02)
        ...
        def minusLW(W):
            ...
            def compute_opto_error_nonlinear(W):
                Wmx,Wmy,Wsx,Wsy,s02,K,kappa,T,XX,XXp,Eta,Xi,h1,h2,h3,Eta1,Eta2,Eta3 = parse_W(W)
                Eta12 = np.concatenate((Eta1,Eta2,Eta3),axis=0)
                Xi12 = np.concatenate((Xi,Xi,Xi),axis=0)
                XX12 = np.concatenate((XX,XX,XX),axis=0)
                fval12 = compute_f_(Eta12,Xi12,s02)

                fval = compute_f_(Eta,Xi,s02)
                dYY12 = fval12 - np.concatenate((fval,fval,fval),axis=0)
                dYYterm = np.sum((dYY12[opto_mask] - dYY[opto_mask])**2)

                dHH = np.zeros((nN,nQ*nS*nT))
                dHH[:,np.arange(2,nQ*nS*nT,nQ)] = 1
                dHH12 = np.concatenate((dHH*h1,dHH*h2),axis=0)
                dHH = np.zeros((nN,nQ*nS*nT))
                dHH[:,np.arange(1,nQ*nS*nT,nQ)] = 1
                dHH12 = np.concatenate((dHH12,dHH*h3),axis=0)
                Eta12perf = u_fn(XX12,fval12,Wmx,Wmy,K,kappa,T) + dHH12
                Eta12term = np.sum((Eta12perf - Eta12)**2)
...
#             fval,fprimeval = compute_f_fprime_t_(W,perturbation) # Eta the mean input per cell, Xi the stdev. input per cell, s02 the baseline variability in input
            ...
            dYYterm,Eta12term = compute_opto_error_nonlinear(W)
