...
Niter = int(1e3)
opto_levels = 1*np.linspace(0,-2,11)
...
vip_halo_l4_npyfile = calnet_base+'vip_halo_data_for_sim.npy'

res_factor = 1#0

def compute_opto_line(scall_halo_l4):
    this_data = np.nanmean(scall_halo_l4,3)
    xdata = this_data[:,:,:,0]
    ydata = this_data[:,:,:,1]
    xdata = xdata.flatten()
    ydata = ydata.flatten()
    ...
    this_data_mean = np.nanmean(this_data,0)
    opto_res = this_data_mean[:,:,1] - this_data_mean[:,:,0]*opto_slope - opto_intercept
    if opto_res.shape[0]==4:
        this_usize = np.array((5,11,25,60))
        target_usize = np.array((5,8,13,22,36,60))
        opto_res_interp = np.zeros((len(target_usize),opto_res.shape[1]))
        for icontrast in range(opto_res.shape[1]):
            interp = sip.interp1d(this_usize,opto_res[:,icontrast])
            opto_res_interp[:,icontrast] = interp(target_usize)
        opto_res = opto_res_interp

    return opto_slope,opto_intercept,opto_res

def adjust_XX(mdl):
    mn = mdl.XX[:,0::2].mean(0,keepdims=True)
    mdl.XX[:,0::2] = mdl.XX[:,0::2]*opto_slope + opto_intercept*mn + res_factor*opto_res.flatten()[:,np.newaxis]*mn
    return mdl
...
def run(fit_lbl,calnet_base=calnet_base,sim_options=sim_options,pool_size=1):
    ...
    weights_files.sort()
    target_file = calnet_base + 'dynamics/vip_halo_l23_as_l4_opto_tavg_%s.npy'%fit_lbl
    #target_file = calnet_base + 'dynamics/vip_halo_l23_as_l4_no_res_opto_tavg_%s.npy'%fit_lbl
    build_models_and_simulate_opto_effects(weights_files,target_file,sim_options=sim_options,pool_size=pool_size)

if __name__=='__main__':

    scallanimal_aligned = np.load(vip_halo_l4_npyfile,allow_pickle=True)[()]['scallanimal_aligned']
    scall_halo_l4 = opto_utils.norm_to_mean_light_off(scallanimal_aligned)
    opto_slope,opto_intercept,opto_res = compute_opto_line(scall_halo_l4)
