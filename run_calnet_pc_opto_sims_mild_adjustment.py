...
def build_models(weights_files):
    ...
    for iwt in range(nwt):
        ...
        mdls_no_pcvip[iwt].Wmy[0,2] = mdls_no_pcvip[iwt].Wmy[0,1]
        mdls_no_pcvip[iwt].set_WW()

    mdls_no_svs = [None for iwt in range(nwt)]
    for iwt in range(nwt):
        wtdict = np.load(weights_files[iwt],allow_pickle=True)[()]
        mdls_no_svs[iwt] = cc.ModelOri(wtdict,nT=1)
        mdls_no_svs[iwt].Wmy[1,2] = 0
        mdls_no_svs[iwt].Wmy[2,1] = 0
        mdls_no_svs[iwt].set_WW()
        
    return mdls,mdls_no_pcsst,mdls_no_pcvip,mdls_no_vipbias,mdls_no_svs
...
def build_models_and_simulate_opto_effects(weights_files,target_file,sim_options=sim_options,pool_size=1):
    ...
    keys = ['YY_opto','YY_opto_tavg_no_pcsst','YY_opto_tavg_no_pcvip','YY_opto_tavg_no_vipbias','YY_opto_tavg_no_svs']
...
def run(fit_lbl,calnet_base=calnet_base,sim_options=sim_options,pool_size=1,target_prefix='pc_opto_tavg_mild',lcutoff=10):
...
def run_one_and_both(pool_size,indicator,calnet_base=calnet_base):
    ...
    run(fit_lbl,pool_size=pool_size,calnet_base=calnet_base,sim_options=sim_options,target_prefix='pc_both_pixels_opto_tavg_mild')
    if indicator:
        sim_options['both_pixels'] = False
        run(fit_lbl,pool_size=pool_size,calnet_base=calnet_base,sim_options=sim_options,target_prefix='pc_one_pixel_opto_tavg_mild')
