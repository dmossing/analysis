...
#opto_levels = 1*np.linspace(-1,1,3)
opto_levels = 1*np.linspace(-0.4,0.4,2)
dt = 1e-1
sim_options = {}
...
def run_on_mdl(mdl,sim_options):
    ...
    this_YY_opto = dyn.compute_steady_state_Model(mdl,Niter=Niter,fix_dim=1,inj_mag=opto_levels,sim_type='inj',dt=dt)
...
def run(fit_lbl,calnet_base=calnet_base,sim_options=sim_options,pool_size=1):
    ...
    weights_files.sort()
    target_file = calnet_base + 'dynamics/sst_opto_tavg_wpcpv_%s.npy'%fit_lbl
