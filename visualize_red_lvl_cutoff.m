function visualize_red_lvl_cutoff(imgred,msk,red_lvl)
imgred = norm01(imgred,1);
options.bkgd = 0;
options.colormap = @(x)gray(x);
% nprct = 100;
% red_map = zeros([size(imgred) nprct]);
msk = msk-imerode(msk,strel('disk',1));
disp('computed masks')
%     aux = gen_color_coded_map(msk>0,red_lvl>prctile(red_lvl,i-1),options);
red_map = cumsum(msk(:,:,sort_by(-red_lvl)),3)>0; %aux(:,:,1);
