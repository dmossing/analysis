...
def compute_opto_line(scall_chrimson_l4):
    this_data = opto_utils.norm_to_mean_light_off(np.nanmean(scall_chrimson_l4,3))
    xdata = this_data[:,:,:,0]
    ydata = this_data[:,:,:,1]
    xdata = xdata.flatten()
    ydata = ydata.flatten()
    ...
    print((opto_slope,opto_intercept))
...
def run(fit_lbl,calnet_base=calnet_base,sim_options=sim_options,pool_size=1):
    ...
    weights_files.sort()
    target_file = calnet_base + 'dynamics/vip_chrimson_l23_as_l4_no_res_opto_tavg_%s.npy'%fit_lbl
    #target_file = calnet_base + 'dynamics/vip_chrimson_l23_as_l4_opto_tavg_%s.npy'%fit_lbl
...
if __name__=='__main__':
    ...
    opto_slope,opto_intercept,opto_res = compute_opto_line(scall_chrimson_l4)
