function rect = sbxAlignmaster(fname,Depth,rect,c)
    ...
    firstframe = sbxchan0(sbxreadpacked(fname,1,1),c);
...
%    end
    ...
    z = sbxchan0(sbxreadpacked(fname,0,1),c);
    ...
    stopat = 0;
    if stopat
        numFrames = min(numFrames,stopat);
    end
    Frames = [Frames(1:numFrames);1:numFrames];
    ...
    parfor jj = 1:numFrames
        ...
        z = double(sbxchan0(sbxreadpacked(fname,Frames(1,jj)-1,1),c));
...
%    s = reshape(sqrt(1/numFrames*(vs - sum(ms.^2,2))),szz);
    ...
    [m,~,T] = sbxAlignpar(fname,thestd,gl,l,Frames,numDepths,rect,c); %Takes about 2.5 minutes
...
%     end
    ...
    for nn = 1:10
        ...
        [m,~,T] = sbxAligniterative(fname,m,rgy,rgx,thestd(rgy,rgx),gl,l,Frames,rect,c);
...
% mask = zeros(szz);
    parfor jj = 1:numFrames
        ...
        z = single(sbxchan0(sbxreadpacked(fname,Frames(1,jj)-1,1),c));
    ...
    parfor jj = 1:numFrames %info.max_idx

        z = double(sbxchan0(sbxreadpacked(fname,jj-1,1),c));
