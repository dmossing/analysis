...
def run_on_mdl(mdl,sim_options):
    ...
    this_YY_opto = dyn.compute_steady_state_Model_multi_inj(mdl,Niter=Niter,fix_dim=fix_dim,inj_mag=opto_levels,sim_type='inj',dt=dt)
...
def run(fit_lbl,calnet_base=calnet_base,sim_options=sim_options,pool_size=1):
    ...
    target_file = calnet_base + 'dynamics/pc_opto_tavg_vip_silencing_%s.npy'%fit_lbl
